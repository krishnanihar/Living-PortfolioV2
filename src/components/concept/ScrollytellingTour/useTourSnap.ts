...
export interface TourSnapState {
  currentIndex: number;
  isAnimating: boolean;
  ...
  progress: number; // 0-1 progress through tour
}

interface UseTourSnapOptions {
  onClose?: () => void;
  onComplete?: () => void;
}

export function useTourSnap(options: UseTourSnapOptions = {}): TourSnapState {
  ...
  // Ref-based lock for synchronous animation blocking
  const animationLockRef = useRef(false);

  // Cooldown timer ref
  const cooldownTimerRef = useRef<NodeJS.Timeout | null>(null);

  // Navigate ref for stable wheel listener
  ...
  const navigate = useCallback((direction: 1 | -1) => {
    ...
    // If going past the last section, trigger complete
    if (newIndex >= TOTAL_SECTIONS) {
      animationLockRef.current = false;
      onComplete?.();
      return;
    }
  ...
  }, [isAnimating, currentIndex, viewportHeight, verticalY, startCooldown, onComplete]);
  ...
  }, [isAnimating, navigate, goToSection, onClose]);

  // Calculate progress (0-1)
  const progress = currentIndex / (TOTAL_SECTIONS - 1);
